...
class _UserHomeScreenState extends ConsumerState<UserHomeScreen> {
    ...
    return categoriesAsync.when(
      data: (categories) {
        ...
        if (_cachedCategories == null ||
            categories.length != _cachedCategories!.length) {
          ...
          for (final priority in priorityCategories) {
            final found = categories.firstWhere((cat) {
              final lowerCat = cat.toLowerCase();
              final lowerPriority = priority.toLowerCase();
              return lowerCat.contains(lowerPriority) ||
                  lowerPriority.contains(lowerCat);
            }, orElse: () => '');
          ...
          if (displayCategories.length < 8) {
            for (final additional in additionalCategories) {
              ...
              final found = categories.firstWhere((cat) {
                ...
                return lowerCat.contains(lowerAdditional) ||
                    lowerAdditional.contains(lowerCat);
              }, orElse: () => '');
              if (found.isNotEmpty && !displayCategories.contains(found)) {
                displayCategories.add(found);
              }
          ...
          if (displayCategories.length < 8) {
            ...
            for (final defaultCat in defaultCategories) {
              ...
              if (!displayCategories.any(
                (c) => c.toLowerCase().contains(defaultCat.toLowerCase()),
              )) {
  ...
  Map<String, dynamic> _getCategoryInfo(String categoryName) {
    ...
    if (lowerName.contains('ăn vặt') || lowerName.contains('snack')) {
    ...
    } else if (lowerName.contains('tráng miệng') ||
    ...
    } else if (lowerName.contains('hoa quả') ||
        lowerName.contains('trái cây') ||
        lowerName.contains('fruit')) {
      return {'icon': Icons.apple, 'color': Colors.redAccent};
    } else if (lowerName.contains('gà rán') ||
        lowerName.contains('fried chicken') ||
        (lowerName.contains('gà') && lowerName.contains('rán'))) {
      ...
      return {'icon': Icons.local_pizza, 'color': Colors.pink};
        ...
        lowerName.contains('lành mạnh')) {
      return {'icon': Icons.spa, 'color': Colors.green};
    } else if (lowerName.contains('soup') ||
        lowerName.contains('canh') ||
        lowerName.contains('cháo')) {
    ...
    } else if (lowerName.contains('pork') ||
        lowerName.contains('heo') ||
        lowerName.contains('lợn')) {
      return {'icon': Icons.restaurant_menu, 'color': Colors.pinkAccent};
    } else if (lowerName.contains('vegetarian') || lowerName.contains('chay')) {
      return {'icon': Icons.eco, 'color': Colors.lightGreen};
    } else if (lowerName.contains('fast food') ||
        lowerName.contains('đồ nhanh')) {
  ...
  Widget _buildFeaturedMerchants(BuildContext context, WidgetRef ref) {
    ...
    final listHeight =
        screenWidth * 0.8; // ~288px trên màn 360px, đủ cho card + shadow.
        ...
        if (_cachedPopularMerchants == null ||
          ...
          if (_userLat != null && _userLng != null) {
            popularMerchants.sort((a, b) {
              ...
              final ratingCompare = (b.rating ?? 0.0).compareTo(
                a.rating ?? 0.0,
              );
              ...
              ...topMerchants.asMap().entries.map((entry) {
                ...
                return Padding(
                  padding: EdgeInsets.only(
                    bottom: index < topMerchants.length - 1 ? 16 : 0,
                  ),
      ...
      child: Row(
        ...
        children: [
          ...
          _buildNavItem(
            ...
            'Cart',
            isActive: false,
            badgeCount: ref.watch(cartProvider.select((items) => items.length)),
          ),
...
class _MerchantCardItem extends ConsumerWidget {
  ...
  const _MerchantCardItem({required this.merchant, this.userLat, this.userLng});
